// Package history persists the last review per repo+branch so a re-review can
// continue from it (confirm fixes, only re-raise what still stands) instead of
// starting from scratch. Stored under ~/.config/silly-review/reviews/, one file
// per repo+branch — never inside the user's repo.
package history
